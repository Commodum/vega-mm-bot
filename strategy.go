...
func RunLpStrategy(walletClient *wallet.Client, dataClient *DataClient) {
...
func SetLiquidityCommitment(walletClient *wallet.Client, dataClient *DataClient) {
	...
	if market := dataClient.s.v[dataClient.c.LpMarket].GetMarket(); market != nil {
		...
		lpSubmission := &commandspb.LiquidityProvisionSubmission{
			...
			Fee:              "0.0005",
			Sells:            []*vegapb.LiquidityOrder{},
			Buys:             []*vegapb.LiquidityOrder{},
...
func getLiquidtyOrders(d decimals, side vegapb.Side) []*vegapb.LiquidityOrder {

	offset := 0.00075
	numOrders := 12

	totalOrderSizeUnits := decimal.NewFromInt(2).Pow(
		decimal.NewFromInt(int64(numOrders + 1)),
	).Sub(decimal.NewFromInt(int64(1))).Div(decimal.NewFromInt(int64(2 - 1)))

	sizeF := func(i int) decimal.Decimal {
		return decimal.NewFromInt(2).Pow(decimal.NewFromInt(i))
	}
