		...
		for i, marketId := range marketIds {
			...
			if market := dataClient.s.v[marketId].GetMarket(); market != nil {
				...
				if marketId == "2c2ea995d7366e423be7604f63ce047aa7186eb030ecc7b77395eae2fcbffcc5" {
					bidVol = decimal.Max(balance.Mul(decimal.NewFromFloat(0.55)).Sub(decimal.Max(openVol.Mul(avgEntryPrice), decimal.NewFromFloat(0))), decimal.NewFromFloat(0))
					askVol = decimal.Max(balance.Mul(decimal.NewFromFloat(0.55)).Add(decimal.Min(openVol.Mul(avgEntryPrice), decimal.NewFromFloat(0))), decimal.NewFromFloat(0))
